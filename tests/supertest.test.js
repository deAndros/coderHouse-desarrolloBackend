...
describe('Ciclo de testing de Power Comics', () => {
  ...
  describe('Apartado de Carts', () => {
    ...
    it('PUT /api/carts/:cid/ debe actualizar la totalidad de los productos de un carrito cuyo ID recibe por parámetro', async () => {
      ...
      //Valido que todos los objetos creados estén dentro del carrito
      /*expect(
        body.payload.updatedCart.products.every((item) => {
          console.log('PRODUCTO FAKE', fakeProducts.product)
          console.log('PRODUCTO EN CARRITO', item.product)
          fakeProducts.product === item.product
        })
      ).to.be.equal(true)*/

      for (let i = 0; i < 3; i++) {
        console.log('PRODUCTO FAKE', fakeProducts[i].product)
        console.log(
          'PRODUCTO EN CARRITO',
        ...
        let productToDelete = fakeProducts[i].product

        console.log('PRODUCTO FAKE')
      }
      ...
      /*for (let i = 0; i < 3; i++) {
        ...
        fakeProducts = fakeProducts.slice(i)
      }*/
